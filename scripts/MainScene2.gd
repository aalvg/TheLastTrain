...
func _input(event):
	if Input.is_action_pressed("fechar_dialogo"):
		...
		$hud/dialogo_chegada.hide()
		$hud/dialogo_metro.hide()

func _on_computador_body_entered(body):
	if body.is_in_group("player"):
		_dialogo_computador.show()
		$hud/dialogo_computador/Dialog_Box/NinePatchRect/MarginContainer/Body_Label/anim.play("dialogo")
...
func _on_saida_body_entered(body):
	get_tree().change_scene("res://MainScene3.tscn")
